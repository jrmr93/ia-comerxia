...
interface SingleOrderCardItemProps {
  ord: any;
  index: number;
  isCollapsed: boolean;
...
const SingleOrderCardItem: React.FC<SingleOrderCardItemProps> = ({
  ord,
  index,
  isCollapsed,
  onToggleCollapse,
  ...
  if (isCollapsed) {
    return (
      <div
        ...
        className={`w-full rounded-2xl border transition-all px-3 sm:px-4 py-2.5 shadow-2xs hover:shadow-xs flex flex-wrap items-center justify-between gap-2.5 bg-white ${cardBorder}`}
      >
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap min-w-0 flex-1">
          <div className="flex items-center space-x-1 font-mono text-xs font-bold text-slate-900 bg-slate-100 px-2.5 py-0.5 rounded-lg border border-slate-200">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <div className="text-right">
            <span className="font-mono font-black text-xs sm:text-sm text-emerald-800">
              ${Number(ord.totalAmount || 0).toFixed(2)} {currency}
            </span>
          </div>
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row items-start gap-3.5 w-full">
        {/* COLUMNA IZQUIERDA: BOTONES DE ACCIÓN UNIFORMES */}
        ...
        <div className="flex-1 min-w-0 w-full space-y-2.5">
          {/* 1. Header Bar: Pedido, Fecha, Estado, Modalidad, Total & Encojer */}
    ...
    <div id="orders-list-container" className="w-full space-y-3">
      ...
      {orders.map((ord, index) => (
        <SingleOrderCardItem
          ...
          ord={ord}
          index={index}
          isCollapsed={collapsedOrderIds.has(String(ord.id))}
          onToggleCollapse={() => handleToggleCardCollapse(ord.id)}
          inventoryItems={inventoryItems}
          purchases={purchases}
          storeConfig={storeConfig}
          currency={currency}
